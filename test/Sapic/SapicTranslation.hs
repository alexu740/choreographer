...
unit_translation_conditional =
  testCase "Translation of conditional (if)" $ do
    ...
    translationForA @?= Right (SIf (TName "x") (TName "y") (SNew (TName "Na") SNil) (SNew (TName "Nb") SNil))
...
unit_translation_write =
  testCase "Write to cell" $ do
    ...
    let translationForA = translateChoreographyToSapicProcess proto agentA
    translationForA @?= Right (SInsert (TFunction CELL [TName "cellA",TName "key"]) (TName "value") SNil)
...
unit_translation_read =
  testCase "Read from cell" $ do
    ...
    let translationForA = translateChoreographyToSapicProcess proto agentA
    translationForA @?= Right (SLookup (TFunction CELL [TName "cellA",TName "key"]) "x" SNil SNil)
